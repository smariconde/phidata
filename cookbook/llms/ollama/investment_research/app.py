...
from assistants import get_investment_research_assistant  # type: ignore
...
st.title("Investment Researcher :bar_chart: ")
st.subheader("This researcher looks for the latest info available and creates a report of a given symbol")
...
def main() -> None:
    # Get LLM Model
    model = (
        st.sidebar.selectbox("Select LLM", options=["llama3:text", "llama3"])
        or "llama3"
    )
    ...
    if "research_assistant" not in st.session_state or st.session_state["research_assistant"] is None:
        research_assistant = get_investment_research_assistant(llm_model=model)
    ...
    if generate_report:
        ...
        if get_chart:
            with st.spinner("Getting Chart"):
                st.image(f'https://charts2-node.finviz.com/chart.ashx?cs=l&t={ticker_to_research}&tf=d&s=linear&ct=candle_stick&tm=d&o[0][ot]=sma&o[0][op]=50&o[0][oc]=FF8F33C6&o[1][ot]=sma&o[1][op]=200&o[1][oc]=DCB3326D&o[2][ot]=sma&o[2][op]=20&o[2][oc]=DC32B363&o[3][ot]=patterns&o[3][op]=&o[3][oc]=000', caption=f'{ticker_to_research.upper()}')
        ...
        if get_purchases:
            with st.status("Getting Insiders Purchases", expanded=True) as status:
                try:    
                    with st.container():
                        purchases_container = st.empty()
                        purchases = ticker.insider_purchases
                        if not purchases.empty:
                            purchases_container.write(purchases)
                            purchases_md = purchases.to_markdown()
                            report_input += "## Insider Purchases\n\n"
                            report_input += "This table outlines last 6 months purchases for the stock.\n\n"
                            report_input += f"{purchases_md}\n"
                        report_input += "---\n"
                    status.update(label="Insider Purchases available", state="complete", expanded=False)  
                except:
                    status.update(label="Insider Purchases Not Available", state="error", expanded=False)
                    pass          
